...
generated_path = bfs(generated_graph, 1, global_node_count)
print (generated_path)
with open('Nodes.txt', 'w', encoding='utf-8') as file:
    for dic in visited_nodes:
        dic = dic.tolist()
        data=json.dumps(dic) 
        file.write(data)
        file.write("\n")
with open('NodesInfo.txt', 'w', encoding='utf-8') as file:
    for dic in generated_graph:
        data=json.dumps(generated_graph[dic]) 
        file.write(data)
        file.write("\n")
with open('NodesPath.txt', 'w', encoding='utf-8') as file:
    for dic in generated_path:
        # dic = dic.tolist()
        data=json.dumps(dic) 
        file.write(data)
        file.write("\n")
# with open('NodesPath', 'w') as fout:
#     json.dump(bfs(generated_graph, 1, global_node_count), fout)